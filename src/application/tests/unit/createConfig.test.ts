...
describe("createConfig", () => {
  ...
  it("createConfig with invalid configuration payload", () => {
    // @ts-ignore
    expect(() => createConfig(pathConfig, { TEST: "test" })).toThrow(
      ErrorTexts.TRY_TO_SET_INVALID_CONFIG
    );
  });

  it("createConfig with empty INSEE_API_KEY", () => {
    expect(() => createConfig(pathConfig, { INSEE_API_KEY: "" })).toThrow(
      ErrorTexts.INSEE_API_KEY_IS_EMPTY
    );
  });
